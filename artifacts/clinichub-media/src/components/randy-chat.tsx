...
const AVATAR_SRC = "/avatars/randy.jpg";
...
export function RandyChat() {
  ...
  const runReply = useCallback(
    async (userText: string, chipId?: DiscoveryChipId) => {
      ...
      try {
        ...
        const showCal = showHandoff && (nextTurns >= 2 || Boolean(reply.offerHandoff) || chipId === "talk_human");
  ...
  const handleChip = async (chipId: DiscoveryChipId, label: string) => {
    setMessages((prev) => [...prev, { id: newId(), role: "user", text: label }]);
    await runReply(label, chipId);
  };
  ...
  return (
    <>
      {/* Bottom-right bubble launcher */}
      <button
        type="button"
        onClick={() => openWidget({ reason: "launcher", mode: "chat" })}
        aria-hidden={open}
        tabIndex={open ? -1 : 0}
        className={`fixed bottom-[calc(6rem+env(safe-area-inset-bottom))] right-4 z-[45] inline-flex h-14 items-center gap-2 rounded-full border border-foreground/15 bg-background/95 pl-1.5 pr-4 text-sm font-semibold text-foreground shadow-xl backdrop-blur-md transition-all hover:border-accent focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent md:bottom-6 md:right-6 ${
          open ? "pointer-events-none opacity-0" : "opacity-100"
        }`}
        aria-label="Chat with Randy"
        data-testid="button-open-randy-chat"
      >
        <span className="relative flex size-11 overflow-hidden rounded-full bg-secondary" aria-hidden>
          <img
            src={AVATAR_SRC}
            alt=""
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="fixed z-50 flex flex-col bg-foreground text-background shadow-2xl sm:bottom-6 sm:right-6 sm:h-[min(680px,calc(100dvh-3rem))] sm:w-[400px] sm:border sm:border-background/20 bottom-0 right-0 h-[100dvh] w-full"
            role="dialog"
            aria-modal="false"
            aria-label="Chat with Randy"
          ...
          >
            ...
            <div className="flex items-center justify-between gap-3 border-b border-background/20 p-4 shrink-0">
              <div className="flex items-center gap-3 min-w-0">
                <div className="relative size-11 shrink-0 overflow-hidden rounded-full bg-background/10 ring-1 ring-background/20">
                  <img
                    src={AVATAR_SRC}
                    alt="Randy — cartoon avatar (BirchReserve.net tee, Best Ad mug)"
                    className="size-full object-cover"
                    width={44}
                    height={44}
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-[10px] uppercase tracking-widest text-background/55 truncate">
                    Portfolio point-guard · draft
                  </p>
                </div>
              </div>
              <button
                  ...
                  <div className="flex flex-wrap gap-2" data-testid="randy-suggestion-chips">
                    {SUGGESTION_CHIPS.map((chip) => (
                      <button
                        key={chip.id}
                        type="button"
                        onClick={() => void handleChip(chip.id, chip.label)}
