...
test("public reserve stores the canonical $490 offer and holds the seat without Stripe", async () => {
  ...
  assert.equal(receipt.amountCents, 49000);
  assert.equal(receipt.currency, "usd");
  assert.equal(receipt.offer, "reserve-490");
  ...
  assert.equal(
    row.promotedOffer,
    "Birch Reserve Category seat",
  );
  assert.equal(row.launchWindow, null);
  assert.equal(row.status, "seat_held");
  assert.equal(row.offerKey, "reserve-490");
  assert.equal(row.amountCents, 49000);
